...
Stubs problematic third-party modules and patches heavy dependency classes
*before* ``import proxy``, then exercises the real FastAPI app via
``httpx.ASGITransport`` with the lifespan disabled so no real
systemd/database/hardware/cooling init runs.
"""
